...
class Packer(xdr.Packer):
    ...
    def pack_auth(self, auth):
        ...
        self.pack_opaque(stuff.encode("utf8"))
...
def sendfrag(sock, last, frag):
    ...
    header = x.to_bytes((x.bit_length() + 7)//8, byteorder='big')
...
def recvfrag(sock):
    ...
    if len(header) < 4:
        raise EOFError
    x = int.from_bytes(header, byteorder='big')
    last = ((x & 0x80000000) != 0)
    n = int(x & 0x7fffffff)
    frag = b''
    while n > 0:
...
class Vxi11Unpacker(Unpacker):
    ...
    def unpack_destroy_link_params(self):
        id = self.unpack_int()
        return id

    def unpack_device_write_params(self):
        link = self.unpack_device_link()
        io_timeout = self.unpack_uint()
        lock_timeout = self.unpack_uint()
        flags = self.unpack_int()
        data = self.unpack_opaque()
        return link, io_timeout, lock_timeout, flags, data

class Vxi11Packer(Packer):
    def pack_device_link(self, link):
        self.pack_int(link)

    def pack_device_error(self, device_error):
        self.pack_int(device_error)

    def pack_create_link_resp(self, error, link, abort_port, max_recv_size):
        self.pack_device_error(error)
        self.pack_device_link(link)
        self.pack_int(abort_port)
        self.pack_int(max_recv_size)
        self.pack_int(9999)
        #self.pack_uint(6666)

    def pack_device_write_resp(self, error, size):
        self.pack_int(0)
        self.pack_int(error)
        self.pack_uint(size)
...
def testsvr():
    ...
    #
    class AbortS(TCPServer):
        def handle_10(self):
            params = self.unpacker.unpack_create_link_params()
            logger.debug('AbortS: CREATE_LINK %s', params)
            self.packer.pack_create_link_resp(0, 0, 10000, 10024)

    s = S('', 395183, 1, 0)
    a = AbortS('', 395184, 2, 10024)
    try:
        s.unregister()
    except OSError as msg:
